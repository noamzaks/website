...
description: "We look at all of the rational numbers in $ (0, 1] $ whose denominator is smaller than some $ n \\in \\mathbb N $. Prove that for any two adjacent numbers $ \\frac {p_i} {q_i}, \\frac {p_{i+1}} {q_{i+1}} $ the following holds: $ p_{i+1} q_i - p_i q_{i+1} = 1 $."
ogDescription: "We look at all of the rational numbers in (0, 1] whose denominator is smaller than some natural number n. Prove that for any two adjacent numbers a/b, c/d the following holds: bc - ad = 1 $."
...
Here's another (with the same code) for $ n = 7 $:
...
The solution is based on a fact that any triangle in the integer number plane which doesn't contain any integer points inside, has an area of $ \frac 1 2 $.

If we look at the triangle given by the origin and the integer points $ (p_i, q_i ) $ and $ (p_{i+1}, q_{i+1}) $, we can show that it indeed doesn't contain any integer point.
...
This point's $ x $ value must be between $ p_i $ and $ p_{i+1} $, but by definition there is no fraction with a denominator this small between them!
